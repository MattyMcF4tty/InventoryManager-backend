...
func getPagedItems(page int, pageSize int) ([]schemas.Item, error) {
	client := db.Connect()
	...
	if err != nil {
		return nil, &schemas.CustomError{
			Code:    http.StatusInternalServerError,
	...
	if count == 0 {
		return []schemas.Item{}, nil
	}

	pageEndIndex := page * pageSize
	pageStartIndex := pageEndIndex - pageSize

	if !utils.InRanage(int(count), pageStartIndex, pageEndIndex) {
		return nil, &schemas.CustomError{
	...
	if pageEndIndex > int(count) {
		pageEndIndex = int(count)
	}
